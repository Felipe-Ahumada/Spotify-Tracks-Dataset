...
Una plataforma de streaming, un sello discográfico o un equipo de A&R necesita decidir, con
información limitada, qué canciones promocionar, en qué playlists incluirlas o qué tipo de
producción priorizar. Hoy esas decisiones dependen en gran medida de la reputación previa del
artista o de la intuición del equipo curatorial, lo que dificulta identificar canciones
prometedoras de artistas nuevos o poco conocidos.
...
loudness, tempo, entre otras) y su métrica de popularidad en Spotify. El problema de negocio que
aborda este proyecto es: **¿qué atributos de una canción —principalmente los musicales/de
audio, y en qué medida el género o el contexto de su lanzamiento— se asocian con su
popularidad?**, como insumo para una futura etapa de modelado predictivo.

Es una decisión deliberada de alcance, justificada en la sección de [sesgos y ética](#7-evaluación-de-sesgos-ética-y-privacidad-de-los-datos),
enfocarse en atributos musicales y no en la identidad del artista: un modelo que solo aprenda
"qué tan famoso es ya el artista" no ayuda a descubrir canciones prometedoras nuevas, que es
exactamente el problema de negocio que se quiere resolver.
...
2. **Explorar qué atributos se asocian con la popularidad de una canción** — de audio, de
   género, y de contexto de publicación (release) — y cuantificar la fuerza de esa asociación.
...
| Señal explicativa por grupo de variables (línea base para justificar qué variables usar en el modelo) | — | Atributos de audio: correlación máxima con `popularity` de solo **-0.13** (`instrumentalness`). Género (con codificación adecuada): **0.62**. Es la variable individual más predictiva encontrada hasta ahora. |

KPI a evaluar en la futura etapa de modelamiento (definido ahora, medido después):

| KPI | Meta |
|---|---|
| Error absoluto medio (MAE) del modelo final sobre el conjunto de prueba | Superar la línea base ingenua de predecir siempre el promedio de `popularity` (34,75), cuyo MAE es **16,16** puntos sobre el dataset preparado (83.478 canciones, `popularity` con desviación estándar 19,41). |
...
`track_name`, `track_genre`), su métrica de popularidad en Spotify (`popularity`, 0-100) y 13
características de audio extraídas por Spotify (`duration_ms`, `danceability`, `energy`, `key`,
`loudness`, `mode`, `speechiness`, `acousticness`, `instrumentalness`, `liveness`, `valence`,
`tempo`, `time_signature`). La descripción detallada de cada columna está en
[`docs/spotify_dataset.pdf`](docs/spotify_dataset.pdf), documento de referencia de Spotify citado
a lo largo del EDA en vez de re-derivarlo.
...
**Herramientas colaborativas:** el equipo trabajó principalmente de forma presencial durante las
horas asignadas en la sala de proyectos, coordinando avances y compartiendo hallazgos por
WhatsApp entre sesiones. Dado que el desarrollo se concentró en encuentros presenciales conjuntos
más que en trabajo asíncrono distribuido, no se usó un sistema de control de versiones compartido
(el proyecto no es, a la fecha de este informe, un repositorio git); la coordinación fue
suficiente para el volumen de trabajo, pero es una limitación a mejorar en una futura entrega con
etapas más largas o distribuidas (ver sección 9).
...
El EDA identifica un **segundo nivel de duplicación que la consolidación por `track_id` no
resuelve**: `track_id` identifica un *release* (álbum, single, recopilatorio), no la canción en
...
en 779 de los 3.022 grupos la misma canción tiene `popularity = 0` en un release y `> 0` en otro
— es decir, buena parte del "pico en cero" no es "esta canción no se reproduce", es "este release
...
negocio real (sección 1) — un modelo así sería inútil para descubrir artistas nuevos, que es
justamente el caso de uso que se busca resolver; y (2) reforzaría un sesgo de retroalimentación
...
- **Modelado** (fase 4): entrenar y comparar modelos de regresión sobre `popularity` usando el
  dataset preparado (`X_train_prep`, `X_test_prep`, `y_train`, `y_test`), con el MAE de línea base
  de la sección 3 (16,16) como punto de comparación.
- **Evaluación** (fase 5) y **despliegue** (fase 6), dependientes del modelado.
- **Control de versiones formal**: adoptar git/GitHub para el trabajo distribuido si una futura
  etapa lo requiere (sección 4).
